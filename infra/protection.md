...
The per-resource checklists — what evidence a retirement needs and the exact query or command that
produces it — are `docs/legacy-retirement.md`. This file says what may not be deleted; that one
says what has to be true before something on the shorter list is.
...
| P5  | Every file in `packages/storage-d1/migrations/core/`                                                 | repository                                                                                                                                                                                                                                                                                                                                                                                                    | Applied migrations are immutable (06 §2). U05 moved the directory out of the ingest Worker with `git mv`; filenames and bytes are identical, and `packages/storage-d1/test/migrations.test.ts` proves it.                                           | Only `git mv`, never an edit.                                                                                                                                                    |
...
| G0-06 | Renaming a Worker directory does not change the physical Worker/DO/R2/Queue identity               | covered                                                   | `scripts/resource-ledger.test.ts`: the committed ledger must equal a fresh read of every config, so a changed `name`, bucket or queue fails CI; the rename of `services/evidence-browser` to `services/app` and of `services/observation-pipeline` to `services/processor` is executed, and the same suite compares the seven moved configs against their identity lines and digests from before the move |
| G0-07 | DO migration tag and class still address the same state                                            | covered (repository side) + runbook                       | `scripts/resource-ledger.test.ts` asserts every DO class has a tag or an explicit sqlite export, and that the executed directory renames changed no tag, class or binding; that the namespace still holds the same state is verified at the first deploy under supervision                                                                                                                                |
...
| G0-09 | Losing only READ leaves CORE protected rows, digests and DATA originals unchanged                  | covered (U11)                                             | `services/processor/test/read-projection.test.ts`: every READ table is dropped, the read migrations re-applied and the projection rebuilt, while the CORE rows, the CORE digest and the DATA objects are asserted unchanged                                                                                                                                                                               |
| G0-10 | Originals that exist only in a legacy bucket are not deleted before the copy is verified           | runbook (§1 P7, §4, `docs/legacy-retirement.md` §5)       | The ledger lists every legacy bucket and every Worker that reads it; the digest-level verification is a live check, and the runbook states it as "every legacy object's sha256 has a `raw_objects` row, or the bucket stays"                                                                                                                                                                              |
| G0-11 | A stored report that references READ keeps its fixed body and evidence                             | covered for reports (U11); U16 owns the second-stage READ | `services/app/test/balances-v2-read.test.ts`: the report reader answers from CORE and DATA with every READ table dropped                                                                                                                                                                                                                                                                                  |
| G0-12 | A PoC with a live cron is not deleted just because nothing imports it                              | covered                                                   | `scripts/resource-ledger.test.ts`: every cron and its deployed status is in the ledger, and each directory's live status is asserted against its configs. `docs/legacy-retirement.md` §0 states the rule the checklists follow: the ledger, not the import graph, decides what is live                                                                                                                     |

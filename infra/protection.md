...
| G0-09 | Losing only READ leaves CORE protected rows, digests and DATA originals unchanged                  | covered (U11)                                             | `services/observation-pipeline/test/read-projection.test.ts`: every READ table is dropped, the read migrations re-applied and the projection rebuilt, while the CORE rows, the CORE digest and the DATA objects are asserted unchanged |
...
| G0-11 | A stored report that references READ keeps its fixed body and evidence                             | covered for reports (U11); U16 owns the second-stage READ | `services/evidence-browser/test/balances-v2-read.test.ts`: the report reader answers from CORE and DATA with every READ table dropped                                                                                                  |

...
function selectedLabels(rows: SourceAccount[], filters: RecordFilters): string[] {
  ...
  return [...markup.matchAll(/<option[^>]*selected=""[^>]*>(.*?)<\/option>/gu)].map(
    (match) => match[1]!,
  );
}
...
test("a source and account absent after refresh stay visibly selected", () => {
  ...
  expect(selectedLabels([{ source_id: "source-B", source_account: "account-B" }], filters)).toEqual(
    ["source-A（今回の記録に含まれません）", "account-A · source-A（今回の記録に含まれません）"],
  );
...
test("an absent account remains distinct from all accounts and recovers when rows return", () => {
  ...
  expect(selectedLabels([{ source_id: "source-A", source_account: "account-B" }], filters)).toEqual(
    ["source-A", "口座名未記録 · source-A（今回の記録に含まれません）"],
  );
  expect(selectedLabels([{ source_id: "source-A", source_account: "" }], filters)).toEqual([
    "source-A",
    "口座名未記録 · source-A",
  ]);
  expect(selectedLabels([], EMPTY_FILTERS)).toEqual(["すべての取得元", "すべての口座"]);
});

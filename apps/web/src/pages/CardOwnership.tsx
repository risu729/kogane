...
function OwnershipForm({
  ...
  return (
    <div className="ownership-form">
      ...
      <p id={`owner-help-${side.role}`} className="panel-note">
        誰の記録かを区別する名前です。同じ人には両方の口座で同じ識別名を使ってください。名前だけでは保有者の証明になりません。本人確認番号やパスワードは入力しないでください。
      </p>
      {label.length > 0 && !ownershipReviewPartyRef(partyRef) ? (
        <p role="alert">
          識別名は128文字以内とし、縦線・スラッシュ・制御文字を含めないでください。
        </p>
      ) : null}
      <label className="ownership-evidence">
        <input
          type="checkbox"
          checked={evidence}
...
export function CardOwnershipPage({ proposalId }: { proposalId: string }): ReactNode {
  ...
  if (!features.cardOwnershipReview)
    return <EmptyState>この接続先は保有者の確認を提供していません。</EmptyState>;
  if (query.isPending) return <Loading label="口座と保有者の根拠" />;
  if (query.isError)
    return (
      <ErrorState
        error={query.error}
  ...
  return (
    <>
      ...
      {query.data.sides.map((side) => (
        <Panel key={side.role} id={`ownership-${side.role}`} title={OWNERSHIP_ROLES[side.role]}>
          <CardOwnershipDetails side={side} />
          <OwnershipForm
            key={`${side.mappingRevision}:${side.ownershipRevision}:${query.data.revision}`}
            review={query.data}
            side={side}
          />
        </Panel>

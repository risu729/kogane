...
export async function collectDomesticArtifacts(options: {
  ...
  const session = await loginDomestic(accessToken, options.endpoints.mtsBaseUrl);
...
export async function collectForeignArtifacts(options: {
  ...
  const session = await loginForeign(accessToken, options.endpoints.foreignStockBaseUrl);
  const to = options.to ?? jstDate(new Date());
  const from = options.from ?? addUtcDays(to, -89);
  assertDateRange(from, to);

  const positions = await foreignGraphql(session, "GetSecuritiesBalanceList", SECURITIES_BALANCES, {
    input: {
      countryCode: "US",
  ...
  for (let pageNum = 1; pageNum <= 20; pageNum += 1) {
    const page = await foreignGraphql(session, "GetTradeRecordList", TRADE_RECORD_LIST, {
      input: {
...
export async function callMts(
  ...
  const response = await fetch(new URL("/mtsmobile/commgate", session.mtsBaseUrl), {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw Object.assign(new Error(`SBI MTS ${trCode} failed with HTTP ${response.status}`), {
...
async function loginDomestic(accessToken: string, mtsBaseUrl: string): Promise<DomesticSession> {
  ...
  const header = parseMtsHeader(buffer);
  if (!response.ok) {
    throw Object.assign(new Error(`SBI MTS login failed with HTTP ${response.status}`), {
      httpStatus: response.status,
    });
  }
  if (header.resultCode && header.resultCode !== "000000") {
...
async function loginForeign(accessToken: string, baseUrl: string): Promise<ForeignSession> {
  const normalizedBase = requiredHttpsBase(baseUrl, "SBI foreign stock base URL");
  ...
  const response = await fetch(new URL("account/authentication:ssoLogin", restUrl), {
    method: "POST",
    headers: {
  ...
  if (!response.ok) {
    throw Object.assign(new Error(`SBI foreign SSO failed with HTTP ${response.status}`), {
      httpStatus: response.status,
    });
  }
  const body = parseJsonObject(text, "SBI foreign SSO");
  const sessionId = response.headers.get("Set-Session") ?? optionalString(body["sessionId"]);
  const accountId = response.headers.get("Account-Id") ?? optionalString(body["accountId"]);
...
async function fetchForeignHash(options: {
  ...
  const text = await response.text();
  if (!response.ok) {
    throw Object.assign(new Error(`SBI foreign hash request failed with HTTP ${response.status}`), {
      httpStatus: response.status,
    });
  }
...
async function foreignGraphql(
  ...
  const text = await response.text();
  if (!response.ok) {
    throw new Error(`SBI foreign GraphQL ${operationName} failed with HTTP ${response.status}`);
  }

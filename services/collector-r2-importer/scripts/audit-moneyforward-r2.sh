...
port="${MONEYFORWARD_AUDIT_PORT:-8977}"
...
(
  cd -- "${service_dir}"
  npx wrangler dev --config wrangler.audit-moneyforward.jsonc \
...
while true; do
  ...
  jq -e '
    type == "object" and
    .schemaVersion == "moneyforward-r2-aggregate-audit-v1" and
    ...
    ((.auditedManifestCount == 1) ==
      (has("manifestStatus") and has("artifactCount") and
       has("accountDetailCount") and has("monthlyFragmentCount"))) and
    ([keys[]] - ["schemaVersion","scannedObjectCount","auditedManifestCount",
      "skippedObjectCount","failedManifestCount","nextCursor","truncated",
      "failureCode","manifestStatus","artifactCount","accountDetailCount",
      "monthlyFragmentCount"] | length == 0)
  ' <<<"${page}" >/dev/null || {
  ...
  if [[ "$(jq -r '.auditedManifestCount' <<<"${page}")" == 1 ]]; then
    ...
    accounts=$((accounts + $(jq -r '.accountDetailCount' <<<"${page}")))
    fragments=$((fragments + $(jq -r '.monthlyFragmentCount' <<<"${page}")))
  ...
  next_cursor="$(jq -r '.nextCursor // ""' <<<"${page}")"
  if [[ -n "${next_cursor}" && "${next_cursor}" == "${cursor}" ]]; then
    printf 'audit cursor did not advance\n' >&2
    exit 1
  fi
  cursor="${next_cursor}"
  [[ -n "${cursor}" ]] || break
...
jq -nc \
  ...
  --argjson failedStatus "${failed_status}" --argjson artifacts "${artifacts}" \
  --argjson accounts "${accounts}" --argjson fragments "${fragments}" \
  '{schemaVersion:"moneyforward-r2-aggregate-audit-v1",source:"moneyforward-me",
    ...
    validatedTotals:{artifacts:$artifacts,accountDetails:$accounts,monthlyFragments:$fragments}}'

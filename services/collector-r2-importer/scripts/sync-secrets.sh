...
for client_id in collector-r2-sbi collector-r2-sbi-vc collector-r2-sony-bank collector-r2-sbi-shinsei collector-r2-mobile-suica collector-r2-global-pass collector-r2-myjcb collector-r2-moneyforward collector-r2-v-point collector-r2-vpass collector-r2-v-point-pay-email collector-r2-smbc-direct; do
  (
    cd -- "${raw_dir}"
...
moneyforward_secret="$(jq -er '."collector-r2-moneyforward" | select(type == "string" and length >= 20)' <<<"${key_map}")"
...
vpoint_pay_email_secret="$(jq -er '."collector-r2-v-point-pay-email" | select(type == "string" and length >= 20)' <<<"${key_map}")"
smbc_direct_secret="$(jq -er '."collector-r2-smbc-direct" | select(type == "string" and length >= 20)' <<<"${key_map}")"
...
(
  ...
  secrets_json="$(jq -nc \
    ...
    --arg moneyforward "collector-r2-moneyforward.${moneyforward_secret}" \
    --arg vpoint "collector-r2-v-point.${vpoint_secret}" \
    --arg vpass "collector-r2-vpass.${vpass_secret}" \
    --arg vpointPayEmail "collector-r2-v-point-pay-email.${vpoint_pay_email_secret}" \
    --arg smbcDirect "collector-r2-smbc-direct.${smbc_direct_secret}" \
    --arg fingerprint "${fingerprint_secret}" \
    '{RAW_EVIDENCE_TOKEN:$sbi,RAW_EVIDENCE_TOKEN_SBI_VC:$sbiVc,RAW_EVIDENCE_TOKEN_SONY:$sony,RAW_EVIDENCE_TOKEN_SBI_SHINSEI:$sbiShinsei,RAW_EVIDENCE_TOKEN_MOBILE_SUICA:$mobileSuica,RAW_EVIDENCE_TOKEN_GLOBAL_PASS:$globalPass,GLOBAL_PASS_LEGACY_EMPTY_SHA256_ALLOWLIST:$globalPassLegacyEmpty,RAW_EVIDENCE_TOKEN_MYJCB:$myjcb,RAW_EVIDENCE_TOKEN_MONEYFORWARD:$moneyforward,RAW_EVIDENCE_TOKEN_VPOINT:$vpoint,RAW_EVIDENCE_TOKEN_VPASS:$vpass,RAW_EVIDENCE_TOKEN_VPOINT_PAY_EMAIL:$vpointPayEmail,RAW_EVIDENCE_TOKEN_SMBC_DIRECT:$smbcDirect,ORIGIN_FINGERPRINT_KEY:$fingerprint}')"
...
unset key_map sbi_secret sbi_vc_secret sony_secret sbi_shinsei_secret mobile_suica_secret global_pass_secret global_pass_empty_secret myjcb_secret moneyforward_secret vpoint_secret vpass_secret vpoint_pay_email_secret smbc_direct_secret fingerprint_secret

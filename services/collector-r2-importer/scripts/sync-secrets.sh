...
for client_id in collector-r2-sbi collector-r2-sbi-vc collector-r2-sony-bank collector-r2-sbi-shinsei collector-r2-mobile-suica; do
  (
    cd -- "${raw_dir}"
...
(
  ...
  secrets_json="$(jq -nc \
    ...
    '{RAW_EVIDENCE_TOKEN:$sbi,RAW_EVIDENCE_TOKEN_SBI_VC:$sbiVc,RAW_EVIDENCE_TOKEN_SONY:$sony,RAW_EVIDENCE_TOKEN_SBI_SHINSEI:$sbiShinsei,RAW_EVIDENCE_TOKEN_MOBILE_SUICA:$mobileSuica,ORIGIN_FINGERPRINT_KEY:$fingerprint}')"
...
unset key_map sbi_secret sbi_vc_secret sony_secret sbi_shinsei_secret mobile_suica_secret fingerprint_secret

...
import auditWorker from "../src/moneyforward-audit-worker";

describe("MoneyForward aggregate-only R2 audit", () => {
  test("returns bounded aggregate fields without source identifiers", async () => {
  ...
  test("reduces strict validation failures to one stable code", async () => {
    const bucket = {
      ...listBucket({
        objects: [
          {
            key: "raw/moneyforward/2026/09/05/123e4567-e89b-42d3-a456-426614174000/manifest.json",
          },
        ],
        truncated: false,
      }),
    ...
    const response = await auditWorker.fetch(auditRequest(), environment(bucket));
    const body = (await response.json()) as Record<string, unknown>;
    expect(body).toMatchObject({ failedManifestCount: 1, failureCode: "manifest_not_found" });
    expect(forbiddenFields(body)).toEqual([]);
  });

  test("continues across an empty truncated R2 page", async () => {
    const bucket = listBucket({ objects: [], truncated: true, cursor: "opaque-next" });
    const response = await auditWorker.fetch(auditRequest(), environment(bucket));
  ...
  test("the repeatable audit is local, remote-read-only, and bounded", () => {
    const script = readFileSync(
      new URL("../scripts/audit-moneyforward-r2.sh", import.meta.url),
      "utf8",
    );
    const config = JSON.parse(
      readFileSync(new URL("../wrangler.audit-moneyforward.jsonc", import.meta.url), "utf8"),
    ) as Record<string, unknown>;
    expect(script).toContain("wrangler dev");
    expect(script).toContain("pages > 100000");
    expect(script).toContain("audit cursor did not advance");
...
function environment(bucket: R2Bucket): Pick<Env, "MONEYFORWARD_SNAPSHOTS"> {
  return { MONEYFORWARD_SNAPSHOTS: bucket };
}

...
import auditWorker from "../src/smbc-direct-layer-b-audit-worker";

describe("SMBC Direct aggregate-only R2 Layer-B audit", () => {
  test("returns only bounded aggregate fields for a skipped object", async () => {
    const bucket = listBucket({
      objects: [{ key: "raw/smbc-direct/example/balance.normalized.json" }],
      truncated: false,
    });
    const result = await auditWorker.fetch(auditRequest(), {
      SMBC_DIRECT_SNAPSHOTS: bucket,
    });
  ...
  test("reduces all contract/parser failures to one aggregate code", async () => {
    const bucket = {
      ...listBucket({
        objects: [
          {
  ...
  test("the harness is local, remote-read-only, and never deploys or mutates R2", () => {
    const script = readFileSync(
      new URL("../scripts/audit-smbc-direct-layer-b-r2.sh", import.meta.url),
      "utf8",
    );
    const config = JSON.parse(
      readFileSync(new URL("../wrangler.audit-smbc-direct-layer-b.jsonc", import.meta.url), "utf8"),

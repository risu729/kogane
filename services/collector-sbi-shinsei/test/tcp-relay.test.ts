...
function harness(
  ...
  class Peer extends EventTarget {
    ...
    readyState = 1;
    send(value: unknown) {
      if (options.sendFails) throw new Error("private-send-body");
  ...
  const readable = new ReadableStream<Uint8Array>({
    ...
    cancel() {
      if (options.cleanupRejects) throw new Error("private-cancel-body");
    },
  });
  const writable = new WritableStream<Uint8Array>({
    ...
    abort() {
      if (options.cleanupRejects) throw new Error("private-abort-body");
    },
  });
...
describe("TCP relay lifecycle", () => {
  test("peer teardown handles closed/close/abort/cancel rejections without runtime errors", async () => {
    ...
    expect(h.logs).toContainEqual(
      expect.objectContaining({
        stage: "socket-close",
        outcome: "expected-close",
        closeReason: "peer-close",
      }),
    );
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", outcome: "closed", transportFailed: false }),
    );
  ...
  test("peer close traces only its bounded protocol code and clean flag, never its reason", async () => {
    const h = harness();
    h.peer.dispatchEvent(
      Object.assign(new Event("close"), {
        code: 1006,
        wasClean: false,
        reason: "private-token-and-url",
      }),
    );
    await h.drain();
    expect(h.logs).toContainEqual(
      expect.objectContaining({
        stage: "websocket-peer-close",
        outcome: "received",
        peerCloseCode: 1006,
        peerWasClean: false,
      }),
    ...
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", peerCloseCode: 1006, peerWasClean: false }),
    );
  ...
  test("a transport rejection before peer closure remains an error", async () => {
    ...
    await h.drain();
    expect(h.logs.filter((log) => log.level === "error")).toContainEqual(
      expect.objectContaining({ stage: "socket-closed", outcome: "failed" }),
    );
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", outcome: "failed", transportFailed: true }),
    );
  ...
  test("a failed write is retained once and is not replayed by later messages", async () => {
    const h = harness({
      write: async () => {
        throw new Error("private-write-body");
      },
    });
    h.peer.message();
    await h.drain();
    ...
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", outcome: "failed" }),
    );
  });
  ...
  test("peer close during an in-flight write drains cleanup without duplicate close or loss of correlation", async () => {
    ...
    expect(h.logs.filter((log) => log.level === "error")).toHaveLength(0);
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", outcome: "closed" }),
    );
    expect(h.writable.locked).toBe(false);
    ...
    expect(h.logs).toContainEqual(
      expect.objectContaining({
        ...
        pendingWrites: 0,
        cleanupDurationMs: expect.any(Number),
      }),
    );
  });
  ...
  test("bidirectional metrics distinguish received, completed writes and queued frames", async () => {
    ...
    expect(terminal).toMatchObject({
      ...
      wsReceivedBytes: 5,
      wsReceivedFrames: 2,
      upstreamWrittenBytes: 5,
      upstreamWrittenChunks: 2,
      upstreamReadBytes: 4,
      upstreamReadChunks: 2,
      wsQueuedBytes: 4,
      wsQueuedFrames: 2,
      pendingWrites: 0,
      peakPendingWrites: 2,
  ...
  test("empty frames never create an upstream socket", async () => {
    ...
    await Promise.all([
      h.peer.message(""),
      h.peer.message(new ArrayBuffer(0)),
      h.peer.message(new Blob([])),
    ]);
    h.peer.dispatchEvent(new Event("close"));
    await h.drain();
    expect(h.connectCalls()).toBe(0);
  ...
  test("synchronous factory failure is reported before closure and never retried", async () => {
    ...
    expect(h.logs.filter((log) => log.level === "error")).toHaveLength(1);
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "socket-connect", outcome: "failed", socketCreated: false }),
    );
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", outcome: "failed", socketCreated: false }),
    );
    expect(h.peer.closeCodes).toEqual([1011]);
  ...
  test("socket-open rejection is owned immediately and remains a transport failure", async () => {
    ...
    expect(h.closeCalls()).toBe(1);
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "socket-open", outcome: "failed", socketCreated: true }),
    );
    expect(h.logs).toContainEqual(
      expect.objectContaining({ stage: "terminal", outcome: "failed" }),
    );

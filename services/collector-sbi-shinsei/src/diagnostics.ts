...
const FIXED_STAGES = new Set([
  ...
  "container-busy",
  "container-internal",
  "navigation-page-unavailable",
  "navigation-cafis-unavailable",
  "login-form-unavailable",
  "login-button-disabled",
  "login-button-click-failed",
  "ui-login-response-timeout",
  "login-json",
  "login-shape",
  "login-rejected",
  "security-connect-timeout",
  "security-connect-shape",
  "validate-token-shape",
  "validate-token-result",
  "read-failed",
  "core-read-result-code",
  "unexpected-origin",
  "missing-input",
...
const READ_STAGES =
  "login|security-connect|validate-token|top-balances|balance-summary|exchange-rate|yen-deposit";
const READ_FAILURE = new RegExp(
  `^(${READ_STAGES})-(network|content-type|oversize|json|shape|invalid-token|result-code|failed|http-(?:0|[1-5][0-9]{2}))$`,
  "u",
);
...
export function browserDiagnostics(stage: string): NonNullable<CollectionFailure["diagnostics"]> {
  const known =
    FIXED_STAGES.has(stage) ||
    READ_FAILURE.test(stage) ||
...
export class ContainerResponseError extends Error {
  constructor(
    ...
    readonly responseReason = "unclassified-response",
  ) {
...
export function containerLifecycleDetails(error: unknown): Record<string, string | number> {
  const details: Record<string, string | number> = {
    ...
    reason: "unknown-lifecycle-error",
  };
  try {
    const message = error instanceof Error ? error.message : typeof error === "string" ? error : "";
    const exit =
      /^(container exited with unexpected exit code|runtime signalled the container to exit): ([0-9]{1,3})$/u.exec(
        message,
      );
    if (exit && Number(exit[2]) <= 255) {
      details.reason =
        exit[1] === "container exited with unexpected exit code"
          ? "process-exit"
          : "runtime-signal";
    ...
    /* A hostile accessor must not break the lifecycle hook. */
  }
  return details;
}
...
export async function containerResponseReason(
  response: Response,
  timeoutMs = 1_000,
): Promise<string> {
  try {
    return await inspectContainerResponse(response, timeoutMs);
  } catch {
  ...
  try {
    return await Promise.race([
      ...
      new Promise<string>((resolve) => {
        timeout = setTimeout(() => resolve("unclassified-response"), timeoutMs);
      }),
    ]);
  } finally {
...
export function failure(
  ...
  const diagnostics: NonNullable<CollectionFailure["diagnostics"]> =
    error instanceof BrowserCollectionError
      ? {
          ...browserDiagnostics(error.stage),
          authenticationAttempted: error.authenticationAttempted === true,
        }
      : {
          stage: staging
            ? "staging-write"
  ...
  if (
    ...
    if (["transport-disconnected", "startup-failed", "proxy-failed"].includes(error.responseReason))
      diagnostics.responseReason = error.responseReason;
  }
  return {
    operation,
    errorType: safeErrorType(error),
...
export function stageDiagnostics(runId: string) {
  ...
  return {
    async step<T>(
      stage: string,
      action: () => Promise<T>,
      resultOutcome?: (value: T) => "success" | "partial" | "failed",
    ): Promise<T> {
      const safeStage =
        WORKER_STAGES.has(stage) || stage === "collection" ? stage : "unknown-worker-stage";
      ...
      try {
        ...
        emitDiagnostic(outcome === "failed" ? "error" : outcome === "partial" ? "warn" : "log", {
          event: "sbi-shinsei-stage",
          runId,
          stage: safeStage,
          outcome,
        ...
        emitDiagnostic(stage === "teardown" ? "warn" : "error", {
          event: "sbi-shinsei-stage",
          runId,
          stage: safeStage,
          outcome: "failed",

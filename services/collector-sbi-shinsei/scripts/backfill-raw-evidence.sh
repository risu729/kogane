...
admin_token_file="${1:-${user_home}/.local/share/kogane/secrets/sbi-shinsei-worker-admin-token}"
collector_url="${SBI_SHINSEI_WORKER_BASE_URL:-https://kogane-sbi-shinsei-collector-poc.takuanimal.workers.dev}"
state_dir="${KOGANE_STATE_DIR:-${user_home}/.local/state/kogane}"
cursor_file="${state_dir}/sbi-shinsei-raw-evidence-backfill.cursor"
stop_after_manifest="${KOGANE_STOP_AFTER_MANIFEST:-0}"
if [[ "${stop_after_manifest}" != "0" && "${stop_after_manifest}" != "1" ]]; then
  printf 'KOGANE_STOP_AFTER_MANIFEST must be 0 or 1\n' >&2
  exit 2
fi
...
while (( page < 100000 )); do
  ...
  if [[ "${stop_after_manifest}" == "1" && "${imported}" -gt 0 ]]; then
    printf '{"stoppedAfterManifest":true,"importedManifests":%d}\n' \
      "${manifest_count}"
    exit 0
  fi
  ...
  cursor="$(jq -er '.nextCursor | select(type == "string" and length > 0)' \
    <<<"${response}")"

...
admin_token_file="${1:-${user_home}/.local/share/kogane/secrets/myjcb-worker-admin-token}"
collector_url="${MYJCB_WORKER_BASE_URL:-https://kogane-myjcb-collector-poc.takuanimal.workers.dev}"
state_dir="${KOGANE_STATE_DIR:-${user_home}/.local/state/kogane}"
cursor_file="${state_dir}/myjcb-raw-evidence-backfill.cursor"
...
while (( page < 100000 )); do
  ...
  if (( failed != 0 )); then
    jq '{page: $page, failedManifestCount, failureCode, failedManifestKey}' \

...
describe("MyJCB: the shared plan stores the collector's redacted pages unchanged", () => {
  test("the shared plan preserves redacted collector bytes and manifest facts", async () => {
    ...
    const expected = Object.fromEntries(
      artifacts.map((entry) => [entry.key.slice(myjcb.PREFIX.length), entry.sha256]),
    );
    ...
    const centralManifest = legacy as Record<string, unknown> & {

 ...
 * again on the bytes that are about to leave the Worker (unified plan 12 §6):
 * no executable or embedding element survived, no attribute that could carry
 * a URL, a session or a credential kept its value, and no card number is left
 * in the text. `redactedStatementHtml` is what makes them hold; this is the
 * assertion that a regression in it fails the run instead of publishing the
 * page. Throws a stable code, never the offending text.
 */

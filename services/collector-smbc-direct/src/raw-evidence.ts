...
export async function backfillStoredRuns(
  ...
  return validateBackfillResult(
    await importerRequest(importer, BACKFILL_PATH, {
      ...(cursor ? { cursor } : {}),
      limit: 1,
    }),
  );
...
function validateImportResult(
  ...
  if (input.status === "deferred") {
    if (
      (input.reason !== "worker_invocation_limit" && input.reason !== "central_inventory_limit") ||
...
function validateBackfillResult(value: unknown): RawEvidenceBackfillPageResult {
  ...
  const failureDetails = input.failureCode !== undefined && input.failedManifestKey !== undefined;
  if (
    outcomeCount > 1 ||

 ...
 * most 40 parse runs (identitySweep's own maximum), and the two unattended
 * lanes publish at most incremental 12 + repair 28 = 40 a tick, so every
 * re-parse is identified on the tick that published it
 * (IDENTITY_RUNS_PER_TICK).
 *
 * The invocation has room for it (docs/observation-lanes.md, measured on
 * synthetic Vpass pages): one job makes at most 16 subrequests (14 D1
 ...
 * which is also the most identitySweep accepts. At its own default of 8, a
 * full repair tick would leave 20 published re-parses without identity on
 * every tick of a drain.
 */
 ...
 * clamped to it, and a test holds every default above to it. */

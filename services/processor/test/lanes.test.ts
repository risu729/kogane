...
test("harness applies every Layer B migration in order through 0048", () => {
  const names = layerBMigrations();
  expect(names[0]).toBe("0017_observation_pipeline.sql");
  expect(names.at(-1)).toBe("0048_statement_fact_indexes.sql");

...
try {
  const queries = {
    ...
    coverage: `SELECT a.source_id,count(DISTINCT a.id) AS parsed_artifacts,count(DISTINCT a.fetch_run_id) AS parsed_runs FROM observation_fetch_artifacts a JOIN parse_runs p ON p.fetch_artifact_id=a.id WHERE p.status='ok' AND p.superseded_by_parse_run_id IS NULL GROUP BY a.source_id`,
  };

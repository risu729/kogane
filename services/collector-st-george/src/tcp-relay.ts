// Vendored from collector-sbi-shinsei/src/tcp-relay.ts: preserve its tested
// stream lifetime/close ownership while keeping this collector independent.
function emitDiagnostic(level: "log" | "warn" | "error", record: object): void {
  try {
...
function safeErrorType(error: unknown): string {
  return error instanceof TypeError ? "type-error" : "transport-error";
}
...
export function startTcpRelay(options: {
  ...
  let relayDeadline: ReturnType<typeof setTimeout> | undefined;
  let queuedBytes = 0;
      ...
      (async () => {
        try {
          ...
          while (!closeReason) {
            ...
            if (metrics.upstreamReadBytes > maxBytes) {
              observeFailure("relay-byte-limit", new Error("relay_limit"));
              break;
            }
  ...
  relayDeadline = setTimeout(
    () => observeFailure("relay-deadline", new Error("relay_limit")),
    180_000,
  );
  ...
  server.addEventListener("message", (event) => {
    ...
    let messageBytes = 0;
    try {
      messageBytes = websocketByteLength(event.data);
      add("wsReceivedBytes", messageBytes);
    } catch {
    ...
    if (metrics.wsReceivedBytes > maxBytes || queuedBytes > maxQueuedBytes) {
      observeFailure("relay-byte-limit", new Error("relay_limit"));
      return;
    }
    add("pendingWrites", 1);
    ...
    writeChain = writeChain
      .then(async () => {
        try {
          ...
          queuedBytes = Math.max(0, queuedBytes - messageBytes);

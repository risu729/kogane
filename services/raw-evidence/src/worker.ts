...
async function route(request: Request, env: WorkerEnv): Promise<Response> {
  ...
  if (request.method === "GET" && url.pathname === "/health") {
    return json({ ok: true, service: "kogane-ingest", apiVersion: "v1", schemaVersion: "0013" });
  }
  ...
  if (request.method === "PUT" && objectMatch) {
    const result = await putObject(request, env, clientId, Number(objectMatch[1]), objectMatch[2]);
  ...
  if (request.method === "GET" && inventoryMatch) {
    return json(
      await getInventoryStatus(env, clientId, Number(inventoryMatch[1]), Number(inventoryMatch[2])),
    );
  }

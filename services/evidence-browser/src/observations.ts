...
export async function overview(store: Store): Promise<Overview> {
...
export async function currentValuations(store: Store): Promise<ValuationRow[]> {
  return (await store.db
    .query(
      `WITH ${SNAPSHOT_CTES}
  ...
  return {
    ...
    parseRuns: await Promise.all(
      runs.map(async ({ warnings_json, ...run }) => ({
        ...run,
        warnings: parseWarnings(warnings_json),
        observations: await observationsForParseRun(store, run.id),
      })),
    ),
  };
}
...
export async function observationsForParseRun(
  store: Store,
  parseRunId: number,
 ...
 * actually sent.
 */
export async function observationDetail(
  store: Store,
...
export async function rawObjectMeta(
  store: Store,
  sha256: string,
): Promise<RawObjectRow | undefined> {
  const row = (await store.db

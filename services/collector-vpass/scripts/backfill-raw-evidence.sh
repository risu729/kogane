...
admin_token_file="${1:-${user_home}/.local/share/kogane/secrets/vpass-worker-admin-token}"
collector_url="${VPASS_WORKER_BASE_URL:-https://kogane-vpass-collector-poc.takuanimal.workers.dev}"
state_dir="${KOGANE_STATE_DIR:-${user_home}/.local/state/kogane}"
cursor_file="${state_dir}/vpass-raw-evidence-backfill.cursor"
admin_token="$(perl - "${admin_token_file}" <<'PERL'
use strict;
use warnings;
use Fcntl qw(:DEFAULT :mode);
my ($path) = @ARGV;
sysopen(my $handle, $path, O_RDONLY | O_NOFOLLOW) or die "admin token file could not be opened safely\n";
...
while (( page < 100000 )); do
  ...
  response="$(curl --config <(printf '%s\n' "${auth_config}") --fail-with-body --silent --show-error --max-time 180 --request POST "${url}")"
  failed="$(jq -er '.failedRecordCount' <<<"${response}")"
  if (( failed != 0 )); then
    jq '{page: $page, failedRecordCount, failureCode}' --argjson page "${page}" <<<"${response}" >&2
    exit 1
  fi
  imported="$(jq -er '.importedRecordCount' <<<"${response}")"
  deferred="$(jq -er '.deferredRecordCount' <<<"${response}")"
  if (( imported + deferred > 1 )); then
    printf 'backfill response has multiple record outcomes\n' >&2
    exit 1
  fi
  record_count=$((record_count + imported))
  deferred_chunk_count=$((deferred_chunk_count + deferred))
  truncated="$(jq -r '.truncated' <<<"${response}")"
  if [[ "${truncated}" == false ]]; then
    rm -f -- "${cursor_file}"
    printf '{"complete":true,"pages":%d,"importedRecords":%d,"deferredChunks":%d}\n' "${page}" "${record_count}" "${deferred_chunk_count}"
    exit 0
  fi
  next_cursor="$(jq -er '.nextCursor | select(type == "string" and length > 0)' <<<"${response}")"
  if [[ "${next_cursor}" == "${cursor}" && "${deferred}" != "1" ]]; then

...
- 国内現物は保有一覧の現在値、個別板／現在値、日足チャートを実口座で取得できた。米国株は試験時点で全保有銘柄の前日終値と日足チャートを取得できたが、`last` と保有一覧の現在値は空だった。
- USD外貨交換の入力レート照会は同じメインサイトsessionで失敗した。口座評価との照合にはMy資産の公式評価額を保存し、汎用UIのFX換算は独立したmarket-data sourceを使う方が安定する。

...
| 6   | `purchase_recognition`   | Only when `PURCHASE_RECOGNITION_ENABLED` is `"1"` or `"true"`; otherwise the stage is not run and logs nothing. Turns adopted Vpass/MyJCB usage rows into purchase and refund events (`docs/economic-events.md`).                                                                                                                |

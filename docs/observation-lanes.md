...
| 10  | `price_promotion`        | Always; no flag. Promotes provider prices to `price_observations` by the closed rule list of `packages/domain/src/price-sources.ts`, at most 500 claims a tick, one cursor per claim kind; logs `scanned`, `promoted`, `basis_unverified`, `unsupported_currency` and `written` (ADR 0020, `docs/calculation-and-reports.md` §1). |
| 11  | `report_job`             | Only when `REPORTS_ENABLED` is `"true"` (`docs/calculation-and-reports.md`).                                                                                                                                                                                                                                                      |
| 12  | `operation_dispatch`     | Always; dispatches only when `OPS_DISPATCH_ENABLED` is `"1"` or `"true"`, otherwise logs `status: "skipped"` (U06/U08, `docs/processor.md`, `docs/ops-api.md`).                                                                                                                                                                   |
| 13  | `decision_outbox`        | Always, and last: it runs after the projections a decision may have invalidated (A09, `docs/change-lifecycle.md`).                                                                                                                                                                                                                |
...
`reward_claims_sweep` promotes, so it follows it. `price_promotion` runs after
`identity_sweep` and right before `report_job`, which reads the prices it
...
`reward_claims_sweep`, `price_promotion`, `operation_dispatch` and
`decision_outbox` — also

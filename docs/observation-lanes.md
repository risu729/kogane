...
| 4   | `balance_projection`   | Always runs and reports itself `skipped` while `BALANCE_PROJECTION_ENABLED` is anything but `"1"` (`docs/balance-read-model.md`).                               |
...
| 7   | `report_job`           | Only when `REPORTS_ENABLED` is `"true"` (`docs/calculation-and-reports.md`).                                                                                    |
| 8   | `operation_dispatch`   | Always; dispatches only when `OPS_DISPATCH_ENABLED` is `"1"` or `"true"`, otherwise logs `status: "skipped"` (U06/U08, `docs/processor.md`, `docs/ops-api.md`). |
| 9   | `decision_outbox`      | Always, and last: it runs after the projections a decision may have invalidated (A09, `docs/change-lifecycle.md`).                                              |

Why those two positions. `collection_scan` registers terminals the shared DATA
...
next one. `operation_dispatch` hands accepted operations to their executor, so
it runs before the outbox — work accepted this tick can still reach it — while
`decision_outbox` keeps its place at the end.

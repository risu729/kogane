...
| -------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `infra/resources.json`           | Machine ledger of every `wrangler*.jsonc` under `services/` and `poc/`: Worker name, config path, D1/R2/KV bindings, Queue producers and consumers, Durable Object classes and migration tags, containers, browser and VPC bindings, service bindings, crons, `email()` handlers, asset directories, var and required-secret **names**, plus each directory's plan disposition and its live status, and the plan rows already executed (what a retired or moved directory became, and the commit that last carried it) | `scripts/resource-ledger.test.ts`                                |
...
- every wrangler config tracked by git appears in the ledger, and every `experiments/*`,
  `services/*` and `poc/*` directory carries a disposition — a new directory or a new config
  cannot be invisible. A directory the plan retires or moves leaves the disposition map and joins
  `completedDispositions`, which records what it became, the commit that last carried its code and
  the live-resource check that allowed the row to be executed (G0-12);

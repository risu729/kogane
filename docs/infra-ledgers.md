...
| -------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------- |
| `infra/resources.json`           | Machine ledger of every `wrangler*.jsonc` under `services/` and `poc/`: Worker name, config path, D1/R2/KV bindings, Queue producers and consumers, Durable Object classes and migration tags, containers, browser and VPC bindings, service bindings, crons, `email()` handlers, asset directories, var and required-secret **names**, plus each directory's plan disposition and its live status | `scripts/resource-ledger.test.ts`                                |

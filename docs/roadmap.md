...
- [The pending/posted job](../services/processor/src/reconciliation-job.ts)
  ...
  payment amount is not silently compared with a purchase amount. MyJCB
  ledgers labelled with the collector's relative `detailMonth-N` fallback are
  not paired, which on the surveyed connection covers its unconfirmed and
  recent confirmed months. The job pages through every published row with a
  scan cursor, but a card's statement month with more than 200 published rows
  (every capture of the month counts) is counted and skipped, not paired.

...
| `publishedParses`            | Adopted for normal display. Today: successful and not superseded. The supersession pointer is the only publication mechanism until PR-05 adds an explicit adoption projection.                                                                                                                                                                                                            | `p.superseded_by_parse_run_id IS NULL AND p.status = 'ok'`                                                                                                                                 |
...
| `completeSnapshotCandidates` | Container datasets whose latest complete capture defines the current snapshot; an empty successful capture is a complete snapshot that means "nothing". Which parses are complete is decided per dataset by its `dataset_snapshot_policies` row (migration 0025): `coverage-v1` reads the stored claim, `legacy-warning-compat-v1` (seeded everywhere) the confined warning-text adapter. | `snapshotCtes({observation_fetch_artifacts, observation_fetch_runs, parse_runs})` from `poc/observation-pipeline/src/snapshot-query.ts`, plus the `CURRENT_SNAPSHOT` membership predicate. |
...
| parsing health (`/api/meta`) | A job-level notion, not a visibility rule: a failed job counts until a newer published parse of the same parser repairs it; retired versions are ignored.                                                                                                                                                                                                                                 | `PARSING_HEALTH_SQL` over `observation_parse_jobs` and `parse_runs`.                                                                                                                       |
| Identity eligibility         | Not part of this package. `services/evidence-browser/src/identity-api.ts` and `observation-organization.ts` read `eligible_identity_runs` (migration 0020) and state the same predicates.                                                                                                                                                                                                 | Unchanged in PR-04; a later PR may move them behind the reader.                                                                                                                            |
...
This PR has no migration and no writer change. Deploy order is reader only:
`services/evidence-browser`. Rollback target is the previous Worker version;
the read views it depends on (0004, 0017) are unchanged. `packages/read-model`
is vendored by relative import, so there is no separate artifact to publish.

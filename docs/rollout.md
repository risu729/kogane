...
| `REWARD_READ_PROJECTION_ENABLED` | processor and app  | off        | The second-stage READ projection (`expiry_estimates`, `conversion_simulations`) is written to and read from `READ` instead of CORE         | everything `READ_PROJECTION_ENABLED` needs, plus that flag on               | Set off on the app, then the processor, exactly like the first-stage flag     |

`COLLECTION_TARGET` arrives with **U09** (one entry per collector) and
`REWARD_READ_PROJECTION_ENABLED` with **U16**; they are in this table because
the table is the whole programme's list, not one branch's. Every other row is
in the configuration on `main` today.
...
5. **App read flags**, each only once its writer has produced something:
   ...
   is published, then `EVENTS_V2_ENABLED`, `REWARDS_V2_ENABLED`.
...
*Not configured:* `automerge.yml` logs `automation app not configured`, exits 0,
...
*Not configured:* `Risk Gate` still runs and still reports on every pull
...
*Not configured:* merging keeps working and only the deployment fails, at its

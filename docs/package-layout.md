...
| `services/collector-*`                     | One deployed Worker per financial source: authentication, collection, and the run's objects and terminal written into the shared `DATA` bucket through `packages/collection`.                                                                                                                                                      | Any package. **Never `poc/`.**                                                                                                                                                                                                                                             |
...
| `tests/fixtures`                           | Synthetic fixtures shared by the parser tests, the identity tests and the experiment, with `MANIFEST.sha256`.                                                                                                                                                                                                                      | Nothing: they are data.                                                                                                                                                                                                                                                    |
...
`services/raw-evidence` and `services/collector-r2-importer`, the legacy
ingest adapter and collector importer, are gone. Their logic moved into the
packages and into the Processor (U05, U08), and the Workers and their resources
were retired on 2026-09-13 ([legacy-retirement.md](legacy-retirement.md)); the
resource ledger's completed dispositions name the commit their code was last
on.

One directory stays where it is on purpose:
...
Tests and scripts are otherwise outside rule 1 on purpose: a test may compare
against the collector that produced the bytes it checks, as the retired
importer's audit tests did.

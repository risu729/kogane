...
| `hono`                                 | 4.13.7             | MIT               | `services/evidence-browser`                | The `/api/ops/v1` router mounted inside the existing `fetch` (U06)                                  | Replacing the Worker's default export, or justifying a second Worker                                                          |
| `zod`                                  | 4.6.2              | MIT               | `services/evidence-browser`                | The runtime contract of `/api/ops/v1` and the MCP tools, as **strict** objects                      | Coercion of any kind — no `z.coerce` for amounts, dates or flags; no unknown-key passthrough                                  |
| `jose`                                 | 6.2.11             | MIT               | `services/evidence-browser`                | JWT/JWS verification in `src/auth.ts`                                                               | Hand-rolled crypto anywhere else                                                                                              |
| `parse5`                               | 8.0.1              | MIT               | `packages/parsers`, three Workers, one PoC | HTML parsing inside parsers                                                                         | Anything that would enter a parser digest without a release (see G4-14 below)                                                 |
...
| `miniflare`                            | 5.20260831.0-alpha | MIT               | `services/observation-pipeline` (direct)   | The pipeline test harness                                                                           | Being treated as production evidence of Cloudflare behaviour                                                                  |
...
`@hono/zod-validator` is **not** installed: `services/evidence-browser/src/ops-api.ts`
...
| `@modelcontextprotocol/sdk`     | Not adopted now               | It works on Workers (`webStandardStreamableHttp`, no `node:` imports), but a minimal server bundles to ~396 KB against today's 231-line, zero-dependency adapter with closed schemas. Revisit if sessions, resumability or resources are needed |
...
to this PR; they are not installed yet. When they land, their rows move up.
...
the per-source dependency digests (`services/observation-pipeline/src/releases.ts`).

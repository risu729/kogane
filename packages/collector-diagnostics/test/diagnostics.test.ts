...
describe("safe collector diagnostics", () => {
  test("keeps only bounded operational metadata from a sensitive exception", async () => {
    ...
    const error = Object.assign(
      new Error("https://provider.invalid/account?token=secret-body user@example.test 123456789"),
      {
    ...
    await expect(
      diagnostic.step("statement-collection", () => {
        throw error;
      }),
    ).rejects.toBe(error);
    ...
    expect(JSON.stringify(records)).not.toMatch(
      /secret|provider|user@|123456789|private|stack|cause/u,
    );
  });

  test("classifies only known codes and numeric statuses", () => {
    expect(safeErrorDetails(new Error("collector_http_429"))).toMatchObject({
      category: "http",
      httpStatus: 429,
    });
    expect(safeErrorDetails(new Error("payment 500 for user@example.test"))).toEqual({
      category: "unknown",
      errorType: "Error",
    });
    expect(safeErrorDetails(Object.assign(new Error("x"), { httpStatus: 900 }))).not.toHaveProperty(
      "httpStatus",
    );
    expect(
      safeErrorDetails(new Error("Missing Worker secret: JRE_ID_CREDENTIAL_JSON")),
    ).toMatchObject({ category: "configuration" });
    expect(safeErrorDetails(new Error("history_session_expired"))).toMatchObject({
      category: "authentication",
      code: "history_session_expired",
    });
    expect(
      safeErrorDetails(
        Object.assign(new Error("html with secrets"), {
          name: "StopConditionError",
          code: "collect-credit-export",
        }),
      ),
  ...
  test("exception accessors and proxies cannot replace the original failure", async () => {
    ...
    for (const key of ["name", "message", "httpStatus", "status", "code"]) {
      const error = Object.defineProperty(new Error("private-body"), key, {
        get() {
      ...
      await expect(
        createDiagnostics("vpass", runId).step("session-open", () => {
          throw error;
        }),
      ).rejects.toBe(error);
    }
    const error = new Proxy(new Error("body"), {
      getPrototypeOf() {
  ...
  test("a broken logging sink changes neither values nor thrown errors", async () => {
    ...
    await expect(
      diagnostic.step("history-collection", () => {
        throw error;
      }),
    ).rejects.toBe(error);

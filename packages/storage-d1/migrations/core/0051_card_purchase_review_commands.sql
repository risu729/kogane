-- Add the card purchase review command kinds (ADR 0017, plan 2026-09 §1.1)
-- without rewriting any recorded judgement. This is 0045 statement by
-- statement: the only difference is the kind CHECK on change_plans.kind and
-- operation_receipts.operation_kind, which gains card-purchase.exclude,
-- card-purchase.restore, card-refund.allocate, card-refund.withdraw,
-- card-installment.link and card-installment.unlink. No planner exists for
-- them yet: planning one is refused before any row is written, and later
-- changes only register planners, never another rebuild.
...
-- identical to 0045 (itself 0031 and 0038). No migration from 0046 to 0050
-- touches these tables. Explicit column lists preserve every historical value.
...
CREATE TABLE change_plans_expanded (
 ...
 kind TEXT NOT NULL CHECK(kind IN ('identity.assign','identity.release-override','relation.accept','relation.reject','card-settlement.accept','card-settlement.reject','card-settlement.withdraw','card-purchase.exclude','card-purchase.restore','card-refund.allocate','card-refund.withdraw','card-installment.link','card-installment.unlink')),

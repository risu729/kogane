...
const COLLECTOR_IDENTITIES_BEFORE_THE_PROMOTIONS = [
  ...
  // U09 added the shared-R2 switch to this collector: the `DATA` binding to
  // the central bucket and `COLLECTION_TARGET=legacy`. The Worker name, the
  // legacy bucket, the cron, the queues and the service binding are untouched;
  // the config digest moves with the edit, in the same commit.
  ...
  // U09 added the shared-R2 switch to this collector: the `DATA` binding to
  // the central bucket and `COLLECTION_TARGET=legacy`. The Worker name, the
  // legacy bucket, the cron and the service binding are untouched; the config
  // digest moves with the edit, in the same commit.

 ...
 *
 * A line changes only when a work item deliberately changes that Worker's
 * configuration, in the same commit as the change and with the reason
 * recorded here. U09 gives each collector the `DATA` binding on the shared
 * `kogane-raw-evidence` bucket and the `COLLECTION_TARGET` var (default
 * `legacy`): `r2=` gains `DATA>kogane-raw-evidence`, `vars=` gains
 * `COLLECTION_TARGET`, and the config digest follows. Nothing Cloudflare
 * already addresses -- Worker name, DO class and migration tag, container,
 * tunnel, cron, existing bucket -- moves.
 */

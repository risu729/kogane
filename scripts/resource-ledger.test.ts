...
const COLLECTOR_IDENTITIES_BEFORE_THE_PROMOTIONS = [
  ...
  "kogane-sbi-collector-poc config=wrangler.jsonc live=true role=deployed email=false crons=0 21 * * * d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=- do-migrations=- do-exports=- containers=- browser=- vpc=- services=- assets=- vars=COLLECTOR_SCHEMA_VERSION secrets=- sha256=96b78fac298b1c94ce749d854968b528a79bfbebd38c1233689f2f1790da903f",
  "kogane-sbi-shinsei-collector-poc config=wrangler.jsonc live=true role=deployed email=false crons=0 21 * * * d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=COLLECTOR_CONTAINER>SbiShinseiCollectorContainer do-migrations=v1[sqlite:SbiShinseiCollectorContainer] do-exports=- containers=SbiShinseiCollectorContainer:./Dockerfile:basic:2 browser=- vpc=MESH>tunnel:6b0ccf30-68b2-494e-baa8-f4f9f3e46b33 services=- assets=- vars=COLLECTOR_SCHEMA_VERSION,RELAY_PUBLIC_URL secrets=- sha256=4df8f113f63603b346212d81d24eab50427c9c2c52221c14821de0bddfa9f731",
  "kogane-sbi-vc-session-poc config=wrangler.jsonc live=true role=deployed email=false crons=*/15 * * * *,5 21 * * * d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=SESSION_STATE>SbiVcSessionState do-migrations=v1[sqlite:SbiVcSessionState] do-exports=- containers=- browser=- vpc=- services=- assets=- vars=COLLECTOR_SCHEMA_VERSION secrets=- sha256=d6afc81f555ba29566ba1d9776a8650a9b0f3e1349604f5c8e5e161651d882fc",
  "kogane-smbc-direct-backfill-poc config=wrangler.jsonc live=true role=deployed email=false crons=- d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=BACKFILL_SESSION>SmbcBackfillSession do-migrations=v1[sqlite:SmbcBackfillSession] do-exports=- containers=- browser=- vpc=TAMIA>tunnel:6b0ccf30-68b2-494e-baa8-f4f9f3e46b33 services=- assets=- vars=COLLECTOR_SCHEMA_VERSION,DEFAULT_BACKFILL_FROM,SMBC_DIRECT_BASE_URL,SMBC_DIRECT_LOGIN_BASE_URL secrets=- sha256=bc87b717919fa49ac204c5c9b0e5f3f3b852e3103c4f688b58c8f0af98c4d9f6",
  "kogane-sony-bank-collector-poc config=wrangler.jsonc live=true role=deployed email=false crons=0 21 * * * d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=- do-migrations=- do-exports=- containers=- browser=- vpc=- services=- assets=- vars=COLLECTOR_SCHEMA_VERSION secrets=- sha256=fe664b463e69ecf7daac251f21078754c2e76982457b883af635aa6091dccdb9",
  "kogane-vpass-collector-poc config=wrangler.jsonc live=true role=deployed email=false crons=0 21 * * * d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=- do-migrations=- do-exports=- containers=- browser=- vpc=- services=- assets=- vars=- secrets=- sha256=05d714f985b4f04fe363b1f898fd50af303694f9dfb6336e582b5cd3bfb5f025",
  "kogane-vpoint-collector-poc config=wrangler.jsonc live=true role=deployed email=true crons=15 21 * * * d1=- r2=DATA>kogane-raw-evidence kv=- queue-producers=- queue-consumers=- do=VPOINT_SESSION>VPointSession do-migrations=- do-exports=VPointSession:sqlite containers=- browser=- vpc=- services=- assets=- vars=COLLECTOR_SCHEMA_VERSION,VPOINT_PAY_EMAIL_RECIPIENT secrets=ADMIN_TRIGGER_TOKEN,VPOINT_EMAIL_FORWARD_TO,VPOINT_EMAIL_RECIPIENT,VPOINT_MEMBER_NUMBER sha256=b3c48d61518b7b07bf42539b80fd99cf1aa37707e89c97b6be3f7f9804af86d4",

// The static prose blocks of the dependency resolution ledger
// (`scripts/dependency-ledger.ts`), unified plan U15 / chapter 07 section 5.
//
// They live in a module of their own for one reason: a multi-hundred-line
// template literal sitting in the middle of a generator is a merge hazard. Two
// branches that each add the same note to that file produce two top-level
// declarations of the same name and no conflict marker, because the hunks do
// not overlap - which is exactly what happened once. Two branches that each add
// *this file* collide on the file itself, and a second declaration inside it
// overlaps the first.
//
// A duplicate top-level `const` is rejected by oxc before any rule runs
// ("Identifier `X` has already been declared"), and hk lints every tracked
// TypeScript file, so the duplicate cannot reach main even if a merge produces
// it. `scripts/dependency-ledger.test.ts` pins that guarantee, because nothing
// in this repository type-checks `scripts/`.
//
// The notes are data, not behaviour: each is rendered verbatim into
// `infra/dependency-resolution.md` above the generated tables.
 ...
 */
export const MIGRATION_NOTE = `## Post-workspace resolution (U03)

...
function validateImportResult(
  ...
  const input = value.status === "sealed"
    ? exactRecord(value, [
      "source", "manifestKey", "status", "centralRunId", "artifactCount",
      "sealed", "finalChunkAllObjectsReused",
    ])
    : exactRecord(value, [
  ...
  if (input.sealed !== true || !positiveInteger(input.centralRunId) ||
...
function validateBackfillResult(value: unknown): RawEvidenceBackfillPageResult {
  ...
  const hasFailureDetails = input.failureCode !== undefined &&
...
function safeManifestKey(value: unknown): value is string {
  return typeof value === "string" &&
    /^raw\/prestia-globalpass\/\d{4}\/\d{2}\/\d{2}\/[0-9a-f]{8}-[0-9a-f]{4}-4[0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\/manifest\.json$/u.test(value);
}

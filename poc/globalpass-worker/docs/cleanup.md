...
- NRT Cloudflare Gateway診断用のWorker `placement.region`、`CF_EGRESS` (`cf1:network`) binding、`chrome-stable-no-ua-all-cloudflare-gateway` variant: inactive時のContainer課金はない。十分な間隔を空けたA/Bを行わずTAMIA継続を確定した場合は、この3点をまとめて削除する。診断用Durable Object名`prestia-globalpass-apac-ne-direct-probe-v1`と`v2`のContainer instanceはdestroy済みで、一覧上のinactive metadataだけが残る。

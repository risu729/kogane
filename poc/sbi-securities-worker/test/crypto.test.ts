...
describe("Bitwarden passkey conversion", () => {
  test("accepts both Bitwarden credential ID encodings", () => {
    expect(parseBitwardenCredentialId("00112233-4455-6677-8899-aabbccddeeff"))
      .toEqual(Buffer.from("00112233445566778899aabbccddeeff", "hex"));
    expect(parseBitwardenCredentialId("b64.AAECA_8"))
  ...
  test("creates a verifiable WebAuthn assertion", () => {
    ...
    expect(authenticatorData.subarray(0, 32))
      .toEqual(createHash("sha256").update("example.com").digest());
    ...
    expect(
      verify("sha256", signed, publicKey, base64UrlBuffer(assertion.signature)),
    ).toBe(true);
  });
});
...
test("decrypts SBI mixed OAEP (SHA-256 label, MGF1-SHA1)", () => {
  ...
  expect(decryptPasskeyToken(base64Url(encrypted), privateKeyPem))
...
function mixedOaepEncode(message: Buffer, modulusBytes: number): Buffer {
  ...
  const padding = Buffer.alloc(
    modulusBytes - message.length - 2 * labelHash.length - 2,
  );

...
const ERROR_CODES = new Set(["ENOENT", "EACCES", "EPERM", "EAGAIN", "ENOMEM", "EINVAL", "EMFILE", "ENFILE"]);
const SIGNALS = new Set(["SIGTERM", "SIGKILL", "SIGABRT", "SIGSEGV", "SIGBUS", "SIGILL", "SIGTRAP", "SIGPIPE", "SIGINT"]);
...
export function observeChildProcess(child, name, { relayUrl } = {}) {
  ...
  } catch { /* Never retain or log the URL. */ }
  ...
  function emit(outcome, fields = {}) {
    try {
      console[outcome === "failed" ? "error" : "log"](JSON.stringify({
        event: "sbi-shinsei-child-process", child: kind,
        ...(runId ? { runId } : {}), outcome, phase: stopping ? "teardown" : "collection",
        durationMs: Math.max(0, Date.now() - startedAt), ...fields,
      }));
    } catch { /* Observability must not prevent failure ownership or cleanup. */ }
  }
  ...
  child.on("error", error => {
    ...
    } catch { /* Error messages, paths, arguments and stacks are never inspected. */ }
  ...
  return {
    isStopped: () => exited || child.exitCode !== null || child.signalCode !== null || (failed && child.pid === undefined),
    assertRunning() {

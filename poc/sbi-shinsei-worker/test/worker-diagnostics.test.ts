...
mock.module("@cloudflare/containers", () => ({
  ...
  getContainer: () => ({
    ...
    destroy: async () => {
      destroyCalls++;
      if (destroyFails) throw new Error("Bearer synthetic-secret");
    },
  }),
}));
...
async function trigger(loggingFails = false) {
  ...
  const capture = (value: string) => {
    if (loggingFails) throw new Error("logger unavailable");
...
describe("Shinsei Worker failure logging", () => {
  test("logs the failed stage before teardown and correlates its relay without exposing credentials", async () => {
    handoff = JSON.stringify({
      ok: false,
      stage: "security-connect-timeout",
    ...
    expect(logs[failed]).toMatchObject({
      runId: result.runId,
      phase: "collection",
      diagnostics: { stage: "security-connect-timeout", authenticationAttempted: true },
    });
  ...
  test("a container transport error is attributed to container-request", async () => {
    ...
    expect(logs.find((entry) => entry.event === "sbi-shinsei-collection-failure")).toMatchObject({
      diagnostics: { stage: "container-request" },
    });
    expect(JSON.stringify(logs)).not.toContain("synthetic-secret");
  });
  ...
  test("a rejected container HTTP response is not logged as a successful request", async () => {
    ...
    expect(logs).toContainEqual(
      expect.objectContaining({
        ...
        diagnostics: { stage: "container-request", httpStatus: 503 },
      }),
    );
  });
  ...
  test("teardown errors stay separate and do not turn a successful collection into a failure", async () => {
    ...
    expect(
      logs.find((entry) => entry.event === "sbi-shinsei-container-destroy-failed"),
    ).toMatchObject({ runId: result.runId, phase: "teardown" });
    expect(JSON.stringify(logs)).not.toContain("synthetic-secret");
  });
});

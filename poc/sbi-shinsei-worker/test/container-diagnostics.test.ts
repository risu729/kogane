...
const { createStageDiagnostics } = await import(new URL("../container/stage-diagnostics.mjs", import.meta.url).href);
...
test("container stages correlate login and reads while excluding the relay URL and handoff body", () => {
  ...
  for (const level of ["log", "error"] as const) {
    const spy = spyOn(console, level).mockImplementation(value => records.push(JSON.parse(String(value))));
    restores.push(() => spy.mockRestore());
  }
  const diagnostic = createStageDiagnostics("wss://private-host.invalid/tcp?token=private-token&runId=00000000-0000-4000-8000-000000000001");
  ...
  expect(records).toContainEqual(expect.objectContaining({ stage: "login-session", outcome: "success", runId: "00000000-0000-4000-8000-000000000001" }));
  expect(records).toContainEqual(expect.objectContaining({ stage: "terminal", outcome: "success" }));
...
test("unknown stage and correlation are omitted or replaced and cleanup remains nonfatal", async () => {
  ...
  await diagnostic.cleanup("browser-close", async () => { throw new Error("private-cookie"); });
  expect(records).toContainEqual(expect.objectContaining({ stage: "browser-close", phase: "teardown", outcome: "failed" }));
  expect(records.filter(record => record.stage === "terminal")).toEqual([expect.objectContaining({ outcome: "success" })]);
  expect(JSON.stringify(records)).not.toContain("private-");
});
...
test("a partial handoff stays partial in the stage and terminal logs without changing its bytes", () => {
  ...
  diagnostic.begin("authenticated-reads");
  const handoff = JSON.stringify({ ok: true, responses: { topBalances: "private-response" }, failure: { dataset: "balance-summary", stage: "balance-summary-http-500" } });
  expect(diagnostic.finish(handoff)).toBe(handoff);
  expect(records).toContainEqual(expect.objectContaining({ stage: "terminal", outcome: "partial" }));
...
test("broken logging cannot change collection handoff or prevent cleanup", async () => {
  ...
  expect(diagnostic.finish(handoff)).toBe(handoff);
  let cleaned = false;
  await diagnostic.cleanup("relay-close", async () => { cleaned = true; });

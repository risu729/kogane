...
describe("safe SBI Shinsei diagnostics", () => {
  test("classifies only bounded SDK HTTP500 envelopes and keeps provider text private", async () => {
    ...
    expect(
      failure("collect", new ContainerResponseError(500, "secret"), "container-request").diagnostics
        ?.responseReason,
    ).toBeUndefined();
  });
  test("container lifecycle keeps only exact known reasons and bounded exit codes", () => {
    expect(
      containerLifecycleDetails(new Error("container exited with unexpected exit code: 1")),
    ).toEqual({ errorType: "Error", reason: "process-exit", exitCode: 1 });
    expect(
      containerLifecycleDetails(new Error("runtime signalled the container to exit: 137")),
    ).toEqual({ errorType: "Error", reason: "runtime-signal", exitCode: 137 });
    expect(containerLifecycleDetails(new Error("Network connection lost."))).toEqual({
      errorType: "Error",
    ...
    for (const message of [
      "container exited with unexpected exit code: 999",
      "secret container exited with unexpected exit code: 1",
      "container exited with unexpected exit code: 1 token=secret",
      "constructor",
      "token=secret",
    ]) {
      expect(containerLifecycleDetails(new Error(message))).toEqual({
        errorType: "Error",
        reason: "unknown-lifecycle-error",
      });
    }
    expect(containerStopDetails({ exitCode: "137", reason: "token=secret" })).toEqual({
      reason: "unknown-stop",
    });
    ...
    expect(containerLifecycleDetails(hostile)).toEqual({
      errorType: "Error",
      reason: "unknown-lifecycle-error",
    });
    expect(
  ...
  test("rejects stage-like secrets, arbitrary error names, bodies, and forged status", () => {
    ...
    expect(failure("collect", error, "container-start")).toEqual({
      operation: "collect",
      errorType: "Error",
  ...
  test("classifies known timeout, network and lifecycle stages, without inventing a status", () => {
    ...
    expect(
      failure("collect", new ContainerResponseError(409), "container-request").diagnostics,
    ).toEqual({ stage: "container-request", httpStatus: 409 });
    expect(
      failure("collect", new ContainerResponseError(Infinity)).diagnostics?.httpStatus,
    ).toBeUndefined();
    expect(manifestFailure(failure("r2:normalized", new Error("secret"))).operation).toBe(
      "r2:normalized",
    );
  });
});

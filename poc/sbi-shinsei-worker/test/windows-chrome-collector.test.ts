...
describe("SBI Shinsei same-Chrome-context handoff", () => {
  ...
  test("rejects a partial envelope whose failure is not the next dataset", async () => {
    ...
    }), new Date("2026-08-31T00:00:00.000Z"))).toThrow(
      "collector failure did not match the next dataset",
    );
  });
  ...
  test("retains all strict raw responses when normalization fails", async () => {
    ...
    top.responseParam.activity.responseParam.activityDetails[0]!.postingDate =
      "9999-99-99";
    const result = parseCollectionResult(JSON.stringify({
      ok: true,
      responses: {

...
function provider(total: number, csvStatus = 200, invalidHistory = false) {
  ...
  const fetcher = async (input: string | URL | Request, init?: RequestInit): Promise<Response> => {
    ...
    if (path.endsWith("/input/")) return new Response("", { headers: { "set-cookie": `FSID=${privateValue}` } });
    if (path.endsWith("revision.json")) return new Response("1");
    if (path.endsWith("csrf-token/get")) return Response.json({}, { headers: { "bff-csrf": privateValue } });
    if (path.endsWith("to-customers/login")) return Response.json({ accountInfo: [{ privateValue }] });
    ...
    if (path.includes("ordinary-deposit-transaction-histories")) {
      ...
      return Response.json({
        ...
        transactionHistInfo: Array.from({ length: Math.min(3, Math.max(0, count - body.acquisitionStrtCnt + 1)) }, () => ({ privateValue })),
      });
    }
    if (path.endsWith("debit-sso/login-usage-dtl-inq")) return Response.json({ debitSsoBinDat: privateValue });
    if (path.endsWith("vcfb02001")) return new Response(`<form name="tisdcform" action="/statement;jsessionid=${privateValue}"><input name="r01" value="${privateValue}"><input name="cc" value="${privateValue}"></form>`);
    if (path.startsWith("/statement")) return new Response(`<form name="nablarch_form3"><select name="W131301.referenceDate"><option value="20260930">September</option></select><input type="hidden" name="session" value="${privateValue}"></form>`);
...
function collect(mock: ReturnType<typeof provider>) {
  return collectSonyBank({
    ...
    from: "2026-09-01", to: "2026-09-05", runId, fetcher: mock.fetcher,
  });
}
...
async function capture<T>(task: () => Promise<T>, throwing = false) {
  ...
  const spies = [spyOn(console, "log").mockImplementation(logger), spyOn(console, "error").mockImplementation(logger)];
  try { return { value: await task(), records }; }
...
describe("Sony history CSV follows the official empty-history guard", () => {
  test("zero JPY preserves official JSON and WALLET while skipping all empty CSVs", async () => {
    ...
    expect(JSON.parse(String(value.artifacts.find((artifact) => artifact.dataset === "yen-history-page-0001")?.body))).toEqual({ countCnt: 0, transactionHistInfo: [] });
    expect(value.artifacts.some((artifact) => artifact.dataset === "wallet-history-202609")).toBe(true);
    expect(records).toContainEqual(expect.objectContaining({ runId, stage: "history-csv", currency: "JPY", outcome: "skipped", reason: "zero_transactions", transactionCount: 0 }));
  ...
  test("positive JPY retains exact CSV bytes and logs validated page counts", async () => {
    ...
    expect(new Uint8Array(value.artifacts.find((artifact) => artifact.dataset === "yen-history-csv")!.body as ArrayBuffer)).toEqual(csvBytes);
    expect(records).toContainEqual(expect.objectContaining({ providerOperation: "EABA0600S1fE11", currency: "JPY", page: 2, rowCount: 1, transactionCount: 4 }));
    expect(records).toContainEqual(expect.objectContaining({ phase: "request", providerOperation: "EABA0600S1fE12", currency: "JPY", httpStatus: 200, durationMs: expect.any(Number) }));
    expect(JSON.stringify(records)).not.toContain(privateValue);
  });
  ...
  test("positive-history CSV 500 remains a failure and records count before the failed request", async () => {
    const { value, records } = await capture(async () => {
      try { await collect(provider(1, 500)); }
      catch (error) { return failure("collect", error); }
      throw new Error("expected failure");
    });
    expect(value.diagnostics).toMatchObject({ stage: "history-csv", currency: "JPY", httpStatus: 500 });
    const validated = records.findIndex((record) => record.stage === "history" && record.transactionCount === 1);
    const failed = records.findIndex((record) => record.stage === "history-csv" && record.httpStatus === 500);
  ...
  test("throwing loggers cannot change successful collection or real CSV failure", async () => {
    ...
    const { value } = await capture(async () => {
      try { await collect(provider(1, 503)); }
      catch (error) { return failure("collect", error); }
      throw new Error("expected failure");
    }, true);
  ...
  test("progress allowlist drops forged identifiers, invalid counts and throwing getters", async () => {
    const { records } = await capture(async () => {
      ...
      diagnostics.record(privateValue, "failed", { currency: privateValue, reason: privateValue, page: Infinity, httpStatus: 999 });
      diagnostics.record("history", "completed", { get transactionCount(): number { throw new Error(privateValue); } });
    });
    expect(records).toEqual([{ event: "sony-bank-progress", phase: "collection", stage: "collect", outcome: "failed" }]);
  });
});

...
import { atStage, failure, manifestFailure, SonyBankError, SonyBankStageError } from "../src/diagnostics";
...
describe("safe Sony diagnostics", () => {
  test("retains the CSV event, currency and 500, but no provider code text", () => {
    ...
    expect(result.diagnostics).toEqual({ stage: "history-csv", reason: "http_error", providerOperation: "EABA0600S1fE12", currency: "JPY", httpStatus: 500, providerErrorCount: 1 });
  ...
  test("drops arbitrary names, messages, causes and forged diagnostic fields", () => {
    ...
    expect(failure("collect", forged).diagnostics).toEqual({ stage: "collect", reason: "unexpected_error", providerErrorCount: 100 });
  ...
  test("network errors preserve the actual login step without echoing fetch errors", async () => {
    let caught: unknown;
    try {
      await collectSonyBank({ credential: { branchNum: "001", accountNum: "1234567", loginPwd: "secret" }, from: "2026-09-01", to: "2026-09-05",
        fetcher: async () => { throw new Error("Bearer synthetic-secret"); } });
    } catch (error) { caught = error; }
    expect(failure("collect", caught).diagnostics).toEqual({ stage: "login-page", reason: "network_error", providerOperation: "login-page" });
    ...
    try {
      await collectSonyBank({ credential: { branchNum: "001", accountNum: "1234567", loginPwd: "secret" }, from: "2026-09-01", to: "2026-09-05",
        fetcher: async (input) => {
          ...
          if (path.endsWith("/input/")) return new Response("", { headers: { "set-cookie": "FSID=secret" } });
  ...
  test("unknown exceptions retain only the assigned stage and R2 operations survive", async () => {
    try { await atStage("wallet", async () => { throw new Error("Bearer synthetic-secret"); }); }
    catch (error) { expect(failure("collect", error).diagnostics?.stage).toBe("wallet"); }
    expect(manifestFailure(failure("r2:yen-history-csv", new Error("secret"))).operation).toBe("r2:yen-history-csv");
  });
});

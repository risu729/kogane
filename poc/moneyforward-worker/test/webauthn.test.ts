...
async function syntheticCredential() {
  ...
  const credential: MoneyForwardCredential = {
    ...
    credentialId: "00112233-4455-6677-8899-aabbccddeeff", counter: 0,
    keyValue: Buffer.from(await crypto.subtle.exportKey("pkcs8", pair.privateKey)).toString("base64url"),
  };
...
test("a raw P-256 signature beginning with 0x30 is still encoded as DER", async () => {
  ...
  try {
    ...
    const expected = Buffer.from([0x30, 68, 0x02, 32, ...raw.slice(0, 32), 0x02, 32, ...raw.slice(32)]);
...
test("the returned DER assertion verifies against the generated public key and exact WebAuthn signed bytes", async () => {
  ...
  const assertion = await createAssertion(credential, { challenge: "synthetic-challenge" });
  const response = assertion["response"] as { signature: string; authenticatorData: string; clientDataJSON: string };
  ...
  const key = createPublicKey({ key: Buffer.from(await crypto.subtle.exportKey("spki", publicKey)), format: "der", type: "spki" });
  expect(verify("sha256", signed, key, Buffer.from(response.signature, "base64url"))).toBe(true);
  expect(JSON.parse(clientData.toString())).toEqual({ type: "webauthn.get", challenge: "synthetic-challenge", origin: credential.origin, crossOrigin: false });
});

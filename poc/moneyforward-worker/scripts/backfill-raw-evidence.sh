...
admin_token_file="${1:-${user_home}/.local/state/kogane/moneyforward-worker-admin-token}"
collector_url="${MONEYFORWARD_WORKER_BASE_URL:-https://kogane-moneyforward-collector-poc.takuanimal.workers.dev}"
state_dir="${KOGANE_STATE_DIR:-${user_home}/.local/state/kogane}"
cursor_file="${state_dir}/moneyforward-raw-evidence-backfill.cursor"
...
while (( page < 100000 )); do
  ...
  if [[ "${truncated}" == false ]]; then
    ...
    printf '{"complete":true,"pages":%d,"importedManifests":%d,"deferredChunks":%d}\n' \
      "${page}" "${manifest_count}" "${deferred_chunk_count}"
    exit 0
  fi
  next_cursor="$(jq -er '.nextCursor | select(type == "string" and length > 0 and length <= 12000)' \
    <<<"${response}")"

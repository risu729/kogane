...
if (candidates.length !== 1) {
  ...
  throw new Error(`Expected one SMBC Direct Bitwarden item, found ${candidates.length}: ${labels.join(", ")}`);
}
...
function isSmbcDirectCandidate(value: unknown): value is Item {
  ...
  if (typeof item.name !== "string" || !/^SMBC(?:\s*)ダイレクト$/iu.test(item.name.trim())) return false;
  ...
  const usernameIsAccount = typeof item.login.username === "string"
    && /^\d+$/u.test(item.login.username.trim());

...
test("an unavailable logging sink does not suppress the existing retry alarm", async () => {
  ...
  const values = new Map<string, unknown>([
    ["progress", { phase: "running", from: "2026-08-01", to: "2026-08-31", startedAt: "2026-09-01T00:00:00Z", runId: "00000000-0000-4000-8000-000000000001", completedChunks: 0, retryCount: 0 }],
  ...
  const profile = spyOn(DirectProfile, "import").mockReturnValue({
    getTransactions: async () => { throw new Error("transactions_service_time_unavailable"); },
    export: () => ({}),
  } as unknown as DirectProfile);
  restores.push(() => profile.mockRestore());
  for (const method of ["log", "warn", "error"] as const) {
    const spy = spyOn(console, method).mockImplementation(() => { throw new Error("logging sink unavailable"); });
  ...
  expect(values.get("progress")).toMatchObject({ phase: "running", retryCount: 1, lastErrorCode: "transactions_service_time_unavailable" });

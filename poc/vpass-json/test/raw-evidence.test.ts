...
const RECORD_KEY =
  "vpass/2026/09/05/2026-09-05T00-00-00-000Z/card-001/manifest.json";
...
describe("Vpass raw-evidence service binding", () => {
  ...
  test("durably requeues every deferred continuation until the Service Binding seals", async () => {
    ...
    const queue = {
      send: async (job: VpassImportJob) => { sent.push(job); return {} as QueueSendResponse; },
    } as unknown as Queue<VpassImportJob>;

    await expect(continueStoredRecord(importer, queue, {
      v: 1,
      recordKey: RECORD_KEY,
  ...
  test("does not acknowledge a failed importer call as sealed or requeue malformed state", async () => {
    let sends = 0;
    const queue = {
      send: async () => { sends += 1; return {} as QueueSendResponse; },
    } as unknown as Queue<VpassImportJob>;
    await expect(continueStoredRecord(
      jsonFetcher({ error: "source_validation_failed" }, 409),
      queue,
      { v: 1, recordKey: RECORD_KEY },
    )).rejects.toThrow("raw_evidence_importer_request_failed");
    await expect(continueStoredRecord(
      jsonFetcher({}, 200),
      queue,
      { v: 1, recordKey: `${RECORD_KEY}.invalid` },
  ...
  test("rejects an inconsistent or oversized importer response", async () => {
    await expect(backfillStoredRuns(jsonFetcher({
      source: "vpass",
    ...
    const oversized = {
      fetch: async () => new Response("x".repeat(70 * 1024), {

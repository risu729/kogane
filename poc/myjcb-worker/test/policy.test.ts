...
describe("MyJCB read-only allowlist", () => {
  ...
  test("permits only the observed credit read/export contracts", () => {
    expect(
      allowedUrl(
        "credit-detail",
        new URLSearchParams({ detailMonth: "17", output: "web" }),
      ).pathname,
    ).toBe("/iss-pc/member/details_inquiry/detail.html");
    expect(
      allowedUrl(
        "credit-pdf",
        new URLSearchParams({ detailMonth: "10", output: "pdf" }),
      ).pathname,
    ).toBe("/iss-pc/member/details_inquiry/detailDbPdf.html");
    expect(() =>
      allowedUrl(
        "credit-detail",
        new URLSearchParams({ detailMonth: "18", output: "web" }),
      )
    ).toThrow();
  });
  ...
  test("keeps only the unobserved root-card switch disabled", () => {
    ...
    expect(DEFERRED_READ_ROUTES.map((route) => route.capability)).toEqual([
      "root-card-switch",
    ]);
  });
});

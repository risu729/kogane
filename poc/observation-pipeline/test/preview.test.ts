...
test("isolated preview identifies synthetic data and never creates the regular store", async () => {
  const regularStore = join(import.meta.dir, "..", "state", "kogane-poc.sqlite");
  ...
  try {
    const origin = await Promise.race([
      (async () => {
        ...
        for (;;) {
          ...
          if (chunk.done) throw new Error("Preview exited before becoming ready");
      ...
      new Promise<never>((_, reject) => {
        timer = setTimeout(() => reject(new Error("Preview startup timed out")), 15_000);
      }),
    ]);
    const metadata = await fetch(`${origin}/api/meta`).then((response) => response.json());
    ...
    expect((await fetch(`${origin}/api/transactions`, { method: "POST" })).status).toBe(405);
  } finally {
    clearTimeout(timer);
    child.kill();
    await child.exited;

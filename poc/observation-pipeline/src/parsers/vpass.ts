...
const CUSTOMIZED_BEAN_WITH_ROWS = [
  ...CUSTOMIZED_BEAN_WITH_SETTLEMENT,
  "meisaiList",
];
const ARTIFACT_KEY =
  /^(?:cards\/(card-\d{3})\/)?months\/(\d{6})\/(top|answer)-(\d{3})\.json$/u;
...
export const vpassStatementPage: Parser = {
  ...
  parse(bytes: Uint8Array, artifact: ArtifactMeta): ParseResult {
    ...
    if ((web === undefined) === (customized === undefined))
      throw new Error(
        "vpass statement page must contain exactly one supported family",
      );
...
function parseWeb(
  ...
  if (scope.pageKind !== "top")
    throw new Error("vpass web page must use a top artifact key");
  exactKeys(content, WEB_CONTENT_KEYS, "vpass web content");
  const bean = requiredObject(value, "vpass web bean");
  exactKeyVariant(
    bean,
    [WEB_BEAN_BASE, WEB_BEAN_WITH_BRIDGE],
    "vpass web bean",
  );
  ...
  rows.forEach((entry, index) => {
    ...
    boundedSafeInteger(
      row["shiharaiPatternFlag"],
      `vpass web row ${index}.shiharaiPatternFlag`,
    );
    ...
    if (
      primary === "45" ||
      primary === "4C" ||
      (primary === "4K" && secondary === "002")
    )
      return;
    if (primary !== "4K" || (secondary !== "005" && secondary !== "007"))
      throw new Error(
        `vpass web row ${index} has an unsupported provider subtype`,
      );
    ...
    const asOf = providerDate(
      data[3],
      scope.month,
      `vpass web row ${index}.data[3]`,
    );
    const merchant = boundedString(
      data[4],
      `vpass web row ${index}.data[4]`,
      false,
    );
    const paymentType = boundedString(
      data[6],
      `vpass web row ${index}.data[6]`,
      true,
    );
    ...
    const providerAmount =
      amountText === ""
        ? undefined
        : jpyInteger(amountText, `vpass web row ${index}.data[5]`);
    ...
    const amount =
      providerAmount === undefined
        ? undefined
        : invertLiability(providerAmount);
    observations.push({
      kind: "transaction",
      ...
      extra: {
        ...
        _kogane: {
          ...
          appMapping:
            secondary === "007"
              ? "InstallmentWithCurrency"
              : "InstallmentWithComment",
...
function parseCustomized(
  ...
  exactKeyVariant(
    bean,
    [
      CUSTOMIZED_BEAN_BASE,
      CUSTOMIZED_BEAN_WITH_SETTLEMENT,
      CUSTOMIZED_BEAN_WITH_ROWS,
    ],
  ...
  const observations: TransactionObservation[] = rows.map((entry, index) => {
    ...
    if (
      (saleCode === "5" && providerAmount <= 0) ||
      (saleCode === "6" && providerAmount >= 0)
    )
      throw new Error(
        `vpass customized row ${index} amount conflicts with uriageKbn`,
      );
    const asOf = providerDate(
      row["riyouDate"],
      scope.month,
    ...
    const merchant = boundedString(
      row["kmName"],
      `vpass customized row ${index}.kmName`,
      false,
    );
...
function artifactScope(artifact: ArtifactMeta): Scope {
  ...
  if (cardFromPath !== undefined && cardFromPath !== card)
    throw new Error(
      "vpass statement card identity does not match its fetch unit",
    );
  const month = match[2]!;
...
function providerDate(
  ...
  const year =
    Math.floor(Number(statementMonth.slice(0, 4)) / 100) * 100 +
    Number(match[1]);
...
function boundedString(value: unknown, label: string, empty: boolean): string {
  if (
    ...
    throw new Error(
      `${label} must be a bounded${empty ? "" : " non-empty"} string`,
    );
  return value;
}
...
function exactKeyVariant(
  ...
  if (!variants.some((keys) => sameKeys(value, keys)))
...
function requireSuccessfulRun(artifact: ArtifactMeta): void {
  if (artifact.runStatus !== "success" || artifact.runFailureCount !== 0)
    throw new Error(
      "Vpass observations require a successful failure-free fetch run",
    );
}

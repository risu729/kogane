...
cursor_file="${state_dir}/vpoint-pay-email-raw-evidence-backfill.cursor"
...
while (( page < 100000 )); do
  page=$((page + 1))
  url="${collector_url}/backfill-vpoint-pay-email-raw-evidence?limit=1"
  if [[ -n "${cursor}" ]]; then
  ...
  response="$(curl --config <(printf '%s\n' "${auth_config}") \
    ...
    --request POST "${url}")"
  failed="$(jq -er '.failedPairCount' <<<"${response}")"
  if (( failed != 0 )); then
    jq '{page: $page, failedPairCount, failureCode}' \
      --argjson page "${page}" <<<"${response}" >&2
    exit 1
  fi
  pair_count=$((pair_count + $(jq -er '.importedPairCount' <<<"${response}")))
  skipped_count=$((skipped_count + $(jq -er '.skippedObjectCount' <<<"${response}")))
  truncated="$(jq -r '.truncated' <<<"${response}")"
  if [[ "${truncated}" == false ]]; then
    rm -f -- "${cursor_file}"
    printf '{"complete":true,"pages":%d,"importedPairs":%d,"skippedObjects":%d}\n' \
      "${page}" "${pair_count}" "${skipped_count}"
    exit 0
  fi
  cursor="$(jq -er '.nextCursor | select(type == "string" and length > 0 and length <= 4096)' <<<"${response}")"

...
describe("V Point email login inputs", () => {
  test("normalizes supported member-number formats", () => {
    expect(normalizeMemberNumber("9234-5678-9012-3456")).toBe("9234567890123456");
...
describe("V Point email login flow", () => {
  test.each([false, true])(
    "replays the form chain with logging unavailable=%s",
    async (loggingUnavailable) => {
      ...
      const fetcher = async (
        ...
        switch (`${method} ${url.hostname}${url.pathname}`) {
          ...
          case "POST mypage.tsite.jp/api/balance_info":
            expect(new Headers(init?.headers).get("cookie")).toContain("AUTH=authenticated");
            return Response.json({ status: { code: "0000" }, results: {} });
          default:
      ...
      for (const request of requests.filter(({ method }) => method === "POST")) {
        ...
        expect(request.form.get("org.apache.struts.taglib.html.TOKEN")).toBe("csrf-token");
      }
  ...
  test("rejects an invalid or expired server response", async () => {
    ...
    await expect(challenge.complete("1234")).rejects.toThrow("rejected or expired");
  });
});

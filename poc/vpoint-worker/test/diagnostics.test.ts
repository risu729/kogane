...
function trigger() {
  return new Request<unknown, IncomingRequestCfProperties>("https://collector.test/trigger", {
    method: "POST", headers: { authorization: "Bearer admin-test-only" },
  });
}

describe("V Point safe diagnostics", () => {
  test("untrusted exception names, messages, stacks and response details are omitted", () => {
    const error = Object.assign(new Error(PRIVATE), { name: PRIVATE, status: 500, applicationCode: PRIVATE });
  ...
  test("provider HTTP failure retains status and active read stage without response body", async () => {
    const stages: string[] = [];
    try {
      await collectVPoint({ sessionCookie: `session=${PRIVATE}`,
      ...
      expect(safeFailure(error)).toEqual({ errorType: "VPointError", failureCode: "provider_http_failed", httpStatus: 503 });
      expect(JSON.stringify(safeFailure(error))).not.toContain(PRIVATE);
    }
  });
  ...
  test("malicious provider application status cannot become a diagnostic", async () => {
    try {
      await collectVPoint({ sessionCookie: "session=test",
        fetcher: async () => Response.json({ status: { code: PRIVATE } }),
      });
      throw new Error("expected failure");
    } catch (error) {
      expect(safeFailure(error)).toEqual({ errorType: "VPointApplicationError", failureCode: "provider_application_failed" });
  ...
  test("partial R2 failure is correlated, preserves partial outcome and redacts failure manifest", async () => {
    const logs = captureLogs();
    let manifest = "";
    const env = fixture({ getSession: async () => `session=${PRIVATE}` }, {
    ...
    spyOn(globalThis, "fetch").mockImplementation(Object.assign(async (input: string | URL | Request) => {
      ...
      return Response.json({ status: { code: "0000" }, results:
        path.endsWith("tpoint_history") ? { history: [], total: 0 } : {}, privateBody: PRIVATE });
  ...
  test("terminal manifest write failure has a safe correlated error even without a saved manifest", async () => {
    const logs = captureLogs();
    const env = fixture({ getSession: async () => { throw new Error(PRIVATE); } }, {
      put: async () => { throw new Error(PRIVATE); },
    });
  ...
  test("forwarding failure is logged once with safe message and no email address or body", async () => {
    ...
    const message = Object.assign({} as ForwardableEmailMessage, {
      ...
      forward: async () => { throw new Error(PRIVATE); },
    });
...
describe("logging cannot change collector behavior", () => {
  test("throwing and non-string error properties degrade to safe classification", () => {
    for (const field of ["name", "status", "reasonCode", "applicationCode"]) {
      ...
      Object.defineProperty(error, field, { get() { throw new Error(PRIVATE); } });
      expect(safeFailure(error)).toEqual({ errorType: "UnknownError", failureCode: "operation_failed" });
    }
    expect(safeFailure(Object.assign(new Error(PRIVATE), { name: null }))).toEqual({ errorType: "UnknownError", failureCode: "operation_failed" });
  });

  test("throwing console does not skip email forwarding", async () => {
    let forwards = 0;
    spyOn(console, "log").mockImplementation(() => { throw new Error("logger unavailable"); });
    spyOn(console, "error").mockImplementation(() => { throw new Error("logger unavailable"); });
    const env = Object.assign({} as Env, {
      VPOINT_EMAIL_RECIPIENT: "collector@example.invalid", VPOINT_PAY_EMAIL_RECIPIENT: "pay@example.invalid",
      VPOINT_EMAIL_FORWARD_TO: "mailbox@example.invalid",
    });
    const message = Object.assign({} as ForwardableEmailMessage, {
      to: "other@example.invalid", forward: async () => { forwards++; },
    });
